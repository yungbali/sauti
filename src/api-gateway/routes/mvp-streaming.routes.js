...
function setupMvpStreamingRoutes() {
  ...
  router.get('/:assetId', [
    ...
    query('connectionType').optional().isIn(['cellular', 'wifi', 'wired', 'slow'])
    ...
    try {
      ...
      const result = await streamingMvpService.getStreamingUrls(req.params.assetId, options);
      res.json(result);
    } catch (error) {
  ...
  router.get('/:assetId/manifest', [
    param('assetId').notEmpty().withMessage('Asset ID is required'),
    query('ads').optional().isBoolean().withMessage('Ads parameter must be boolean')
  ], handleValidationErrors, async (req, res) => {
    try {
      ...
      if (includeAds) {
        ...
        const result = await adMvpService.getAdEnabledManifest(assetId, userContext);
        ...
        if (result.adEnabled) {
          ...
          const streamingResult = await streamingMvpService.getStreamingUrls(assetId);
   ...
   */
  router.get('/:assetId/qualities', [
    param('assetId').notEmpty().withMessage('Asset ID is required'),
    query('connectionType').optional().isIn(['cellular', 'wifi', 'wired', 'slow'])
  ], handleValidationErrors, async (req, res) => {
    try {
      const result = await streamingMvpService.getStreamingUrls(req.params.assetId, {
        connectionType: req.query.connectionType
      });
      ...
      res.json({
        ...
        recommendedQuality: result.qualities.find(q => q.recommended)?.label || 'Auto'
      });
    } catch (error) {
